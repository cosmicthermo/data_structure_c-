...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (head_  == nullptr && tail_ == nullptr) {
    ...
    // pointer arrives at one node after the insert point.
    newNode->next = temp;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // and an int.)
  auto nodePtr1 = left.getHeadPtr();
  auto nodePtr2 = right.getHeadPtr();
  while (nodePtr1 && nodePtr2) {
    if (nodePtr1->data <= nodePtr2->data) {
      merged.pushBack(nodePtr1->data);
      nodePtr1 = nodePtr1->next;
    } else {
      merged.pushBack(nodePtr2->data);
